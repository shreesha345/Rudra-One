"""
RudraAnalyst — analytics agent that answers data questions and generates
HTML dashboard artifacts.

Refactored to use the OpenAI-compatible LLM factory (LangChain ChatOpenAI).
"""
...
from backend.services.llm import get_langchain_llm
...
MOCK_HOURLY_STATS = [
    {"hour": h, "avg_volume": v, "busiest_day": d}
    for h, v, d in [
        (0, 12, "Saturday"), (1, 8, "Sunday"), (2, 5, "Sunday"), (3, 4, "Saturday"),
        (4, 3, "Monday"), (5, 6, "Monday"), (6, 15, "Monday"), (7, 35, "Monday"),
        (8, 65, "Monday"), (9, 80, "Tuesday"), (10, 75, "Wednesday"), (11, 70, "Friday"),
        (12, 65, "Friday"), (13, 60, "Friday"), (14, 55, "Thursday"), (15, 65, "Friday"),
        (16, 85, "Friday"), (17, 95, "Friday"), (18, 110, "Saturday"), (19, 105, "Saturday"),
        (20, 90, "Saturday"), (21, 75, "Sunday"), (22, 50, "Sunday"), (23, 30, "Saturday"),
    ]
]
...
class RudraAnalyst:
    def __init__(self):
        if not config.LLM_API_KEY:
            logger.warning("LLM_API_KEY not found. Analytics will not function fully.")
            self.llm = None
        else:
            self.llm = get_langchain_llm(
                model=config.ANALYTICS_LLM_MODEL,
                temperature=config.ANALYTICS_LLM_TEMPERATURE,
            )

    def query_data(self, user_query: str) -> str:
        if not self.llm:
            return "LLM not initialized. Cannot answer query."
        data_context = json.dumps(_full_data(), indent=2)
    ...
    def generate_visual_artifact(self, user_query: str) -> str:
        if not self.llm:
    ...
    def chat(self, user_message: str) -> Dict[str, str]:
        ...
        if "VISUAL" in intent or any(keyword in user_message.upper() for keyword in visual_keywords):
            return {"type": "html", "content": self.generate_visual_artifact(user_message)}
        else:
            return {"type": "text", "content": self.query_data(user_message)}
