...
def format_call_message(insights_data: dict, location_address: str, emergency_type: str) -> str:
    """Format call message from insights data using Gemini or fallback"""
    try:
        if not GOOGLE_API_KEY:
            logger.warning("⚠️ Google API key not configured for call message formatting")
            # Fallback message
        ...
        # Use Gemini to format call message
...
- Keep it concise and urgent"""
            ...
            formatted_message = response.text.strip().replace('"', '').replace("'", "")
            logger.info(f"✅ Call message formatted with Gemini: {formatted_message[:50]}...")
